...
Teaching rationale: ${item.explanation}` : "";
  ...
  var loaded = false;
  var settings = { configured: false, model: "gpt-6-sol" };
  var pending = null;
  var revision = 0;
  var feedback = null;
  ...
  function configure() {
    ...
    else notice("Open AI settings in the Mac application. API credentials are never entered into this browser preview.");
  }
  ...
  window.statsDirectLearn = {
    ...
    reply(value) {
      ...
      pending = null;
      transcriptEntry(state, "assistant", value.text, "OpenAI tutor", { model: value.model, responseID: value.responseID, promptVersion: value.promptVersion, courseSources: value.courseSources ?? [] });
      save();
      render();
    },
    tutorError(value) {
  ...
  if (native) post({ action: "ready" });
    ...
    notice("Browser preview. Use the Mac app for the OpenAI tutor, analyses, R sessions and email drafts.");
  }
})();
