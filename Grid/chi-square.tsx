...
import DataEditor, { CompactSelection, GridCellKind, type GridCell, type GridSelection, type Item } from '@glideapps/glide-data-grid';
...
function App() {
  ...
  return <main>
    ...
    <div className="layout"><section className="data-panel" aria-label="Observed counts">
      ...
      <div className="cell-editor"><label htmlFor="count">Row {selectedRow+1}, column {selectedCol+1}</label><input id="count" aria-label="Selected count" value={cellText} disabled={busy} onChange={e=>setCellText(e.target.value)} onBlur={editCell} onKeyDown={e=>{if(e.key==='Enter'){editCell();e.currentTarget.blur();}}}/></div>
      <div className="canvas" ref={container}><DataEditor width={size.width} height={size.height} columns={table.state.columnLabels.map(title=>({title,width:145}))} rows={countRows} getCellContent={cell} getCellsForSelection={true} rowMarkers="number" rowMarkerWidth={40} headerHeight={35} rowHeight={32} gridSelection={selection} onGridSelectionChange={select} onCellsEdited={items=>{if(busy)return false;table.change(s=>items.forEach(({location:[c,r],value})=>{if(value.kind===GridCellKind.Text)s.counts[r][c]=value.data;}));changed();return true;}} onPaste={(target,values)=>{if(!busy)attempt(()=>{table.pasteValues(target[0],target[1],values);changed();});return false;}} onDelete={s=>{if(!busy && s.current){const area=s.current.range;table.change(next=>{for(let r=area.y;r<area.y+area.height;r++)for(let c=area.x;c<area.x+area.width;c++)next.counts[r][c]='';});changed();}return false;}} smoothScrollX smoothScrollY /></div>
