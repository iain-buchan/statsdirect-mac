...
import {worksheetInput,worksheetRows,enteredInput,pasteMatrix} from './operation-data.mjs';
...
function Prompt({p,source,onSubmit,initial,busy}:{p:any,source:any,onSubmit:(v:any)=>void,initial:any,busy:boolean}) {
  ...
  return <form onSubmit={submit}><fieldset disabled={busy}><h2>{p.prompt==='Enter a value'&&p.kind==='confidence'?'Confidence level':p.prompt==='Enter a value'&&p.kind==='grid'?'Enter the data table':p.prompt||p.title}</h2>{p.rubric&&<p className="rubric">{p.rubric}</p>}{p.error&&<p className="error" role="alert">{p.error}</p>}
    ...
    {error&&<p className="error" role="alert">{error}</p>}<div className="actions"><button className="primary" type="submit">{p.kind==='settings'?'Save defaults':'Continue →'}</button>{p.skip&&<button type="button" onClick={()=>onSubmit({skip:true})}>{p.skip}</button>}</div></fieldset></form>;
}
function App(){
  ...
  return <main><header><div><div className="eyebrow">ANALYSIS / STATSDIRECT</div><h1>{config.title}</h1><p>{isSettings?"Set the defaults used for new analyses.":"Results and charts open as separate reports in the Window menu."}</p></div><button onClick={()=>native('help')}>Method help ↗</button></header>
    ...
    {!isActive&&state.state!=='ready'&&<section><h2>{state.state==='complete'?(isSettings?'Analysis defaults saved':'Analysis complete'):state.state==='cancelled'?(isSettings?'Changes cancelled':'Analysis cancelled'):'Analysis could not finish'}</h2><p className={state.state==='failed'?'error':''}>{state.error|| (state.state==='complete'?(isSettings?'These settings will be used by new analyses, including after restarting StatsDirect.':'Your report is open in its own tab.'):(isSettings?'Your saved defaults are unchanged.':'No final report was created.'))}</p><button className="primary" disabled={busy} onClick={()=>{lastAnswer.current=undefined;setBusy(true);native('start');}}>{isSettings?'Edit defaults':'Run again'}</button></section>}
    {isActive&&<div className="cancel"><button onClick={()=>{setBusy(true);native('cancel');}}>{isSettings?'Cancel':'Cancel analysis'}</button><span>{busy?'Waiting for the engine…':'Answers are checked by the StatsDirect engine.'}</span></div>}
