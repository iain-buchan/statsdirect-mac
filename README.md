...
Version 0.3.1 includes **Help → Learning** and **Learning Options**, personalised exam goals, epidemiology and causal inference, 30 original practice questions, StatsDirect/R examples, tutor-supplied course packs and review-record email drafts. Learners use a **managed tutor connection**, with no OpenAI key entry. The [shared service](TutorService/README.md) holds the administrator's OpenAI credential and enforces usage limits. Hosting and service activation remain pending; the local lessons and practice are available now. See [Learning documentation](Learn/README.md), [course-pack template](Docs/Learn/CoursePack/course-pack-template.json) and [exam sources](Docs/Learn/EXAM-SOURCES.md). Draft scores do not award accreditation or CPD points.
