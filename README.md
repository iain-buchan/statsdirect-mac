...
Download the Mac application ZIP from the [latest published release](https://github.com/iain-buchan/statsdirect-mac/releases/latest). The current preview requires Apple Silicon and macOS 14 or later. It is not yet Developer ID signed or notarised; see the release notes for installation details.

**Help → Check for Updates…** checks the latest published stable release of this GitHub repository. **Automatically Check for Updates** is enabled initially and can be toggled in Help. Background checks run at most once a day, starting after launch and while the app remains open; unchanged versions and transient background failures stay quiet. A newer version offers **View Update**, which opens the release notes and downloads on GitHub without interrupting open work. Downloads and installation are currently manual.

Pushing source changes or building a new local app does not publish an update. Publish a GitHub release with a numeric tag such as `v0.3.10` and attach the verified Mac application archive or installer before announcing that an update is available. Drafts, prereleases, plain commits and Git tags without releases are not updates. With no accessible release, the app says so; an offline, malformed or rate-limited response never claims the app is up to date. No GitHub token or learner credential is bundled. The checker first uses GitHub's [latest-release API](https://docs.github.com/en/rest/releases/releases#get-the-latest-release). If the API host is unreachable or the request fails, it falls back to GitHub's documented [latest-release web address](https://docs.github.com/en/repositories/releasing-projects-on-github/linking-to-releases), validating the final repository/tag URL. If both requests fail, the dialog explains the connection problem and offers Try Again or Open Downloads; it does not claim the app is up to date. Production installation will need the separately planned Apple signing/notarization workflow.
...
**File → New Worksheet** (⌘N) opens a blank 100-row, eight-column worksheet. Double-click to edit, or use the selected-cell value field. Return saves the value and selects the cell below. Arrow keys save an edit and move one cell in the indicated direction. These controls also work in analysis input grids and stop at table edges. Small analysis grids fit their dimensions, so a 2 × 2 count table displays only four entry cells plus headings. Highlighting a 2 × 2 worksheet block before opening the test fills those four cells in the same orientation. Other compatible screen-entry grids also accept selected rectangles. Larger or mismatched selections are not silently cropped; unsafe formula caches and Excel errors are rejected. Recalculation preserves edited answers and validates them through the original engine.
...
**Load help example** fills the 4 × 3 grief/support table (66 observations) and enables expected counts and cell contributions. **Run analysis** shows results above the retained form. **Add to report** appends those results to the active report. The original engine provides Pearson and G tests, exact testing, percentages, expected counts, cell chi-square, trend scores and optional Monte Carlo simulation. Custom scores are supplied through the engine's existing amendment prompt. A fixed simulation seed reproduces results. Method help opens separately.
...
This is table interchange, not a full R workspace editor. Other objects and unsupported columns (such as nested lists or custom classes) are reported as not imported, and are not written by Save RData. Extra application-specific attributes and data-frame subclasses are not retained. Import is limited to 50 MB, 128 tables and 500,000 cells. Conversion runs separately from any open R script sessions, which keep their own variables. If R is missing, StatsDirect offers to download and install it before opening or saving an R data file.

## R sessions

Choose **R → New R Session** or **File → New R Session** (⌘R) to start a session. Use **Window** to switch between sessions and other documents. Each tab starts a separate persistent session using the R installation on this Mac. Edit the script and click **Run script**, or press **⌘Return**. The script editor starts empty. The output pane displays text, warnings, errors and charts; variables remain available between runs. Charts appear when a run finishes, scale to fit the pane and include an **Open plot file** link to the original. Multiple plots and PDF pages are retained in the output history.
...
R-created files are retained in `~/Library/Application Support/StatsDirect Viewer/R Sessions/<session-id>`, shown in the console. Ordinary plot calls use a headless PDF device automatically. New or updated PDF, PNG, JPEG and TIFF files in the session folder (including subfolders) appear inline, including the generated agreement PDF. Devices opened by a script are flushed and closed after each run, including after an error; the display is a static preview, not an interactive graphics device. Interactive console input such as `readline()` is not supported. The R tab is separate from the calculation engine's R-operation integration. R is downloaded on demand rather than bundled with the application.

## Installing R

When an R session, learning example, report R link or R data-file action needs R and none is found, StatsDirect opens **Install R for StatsDirect**. **Download and Install R** downloads the official R Project package, verifies its pinned SHA-256, checks the R publisher's Apple Installer signature and asks macOS to assess the package before opening it in macOS Installer. Download progress, cancellation and retry are built in. **R → Install R…** also starts setup; when R is already available the menu shows **R Is Installed**.

The current Apple Silicon build uses R 4.6.1 for macOS 14 or later (about 101 MB), from [CRAN](https://cran.r-project.org/bin/macosx/). The learner completes the normal macOS Installer steps, including any administrator approval. Returning to StatsDirect checks that Rscript actually works before resuming the requested script or file action. **Check Again** is available if installation is still finishing. Scripts and open documents remain intact if setup is cancelled or postponed. An existing R installation is used without downloading or upgrading it.

The package version, official URL and SHA-256 are pinned in `Sources/RInstallation.swift`; advancing them requires verifying a new official package. Automatic setup is not a privileged silent installation, and managed Macs may require their administrator to complete it. R is detected in the standard R.framework, Apple Silicon Homebrew and Intel Homebrew locations. Network, integrity or signature failures never open the downloaded installer.

## Continue a completed analysis in R

Completed analysis reports include a small **Continue in R** link beside method help at the bottom of the report, also available as **R → Continue Report in R**. Clicking it opens an independent R tab with a generated script and runs it once. The editor remains editable; use Run Script to rerun it or Save Script to keep a standalone `.R` file. Generated scripts are treated as unsaved until saved.

The script contains the completed run's recorded inputs, normalized confidence levels, parameter choices, original scalar results and any output tables. It does not reread the live worksheet, so editing or closing the source form cannot change an earlier report's R analysis. Named `data_frames`, `parameters`, `original_results`, `input_history` and `result_tables` objects remain available for exploration. The R data contain only the selected worksheet block, retaining internal missing cells; `input_history` refers to that block rather than repeating the entire data. If R is missing, setup preserves the generated script and runs it after installation. Additional R packages are not installed automatically.
...
Version 0.3.8 includes **Help → Learning** and **Learning Options**, personalised exam goals, epidemiology and causal inference, 30 original practice questions, StatsDirect/R examples, tutor-supplied course packs and review-record email drafts. Learners choose **Use my ChatGPT** and sign in in their browser, with no API key entry or separate runtime installation. The app bundles the pinned official Codex App Server; the learner's ChatGPT/Codex access and usage allowance apply. Browser sign-in must be completed before sending a live question. The managed-service prototype remains available in the repository for future institutional use. See [Learning documentation](Learn/README.md), [course-pack template](Docs/Learn/CoursePack/course-pack-template.json) and [exam sources](Docs/Learn/EXAM-SOURCES.md). Draft scores do not award accreditation or CPD points.

The tutor can now read current worksheets, selected cells, forms, reports and R output through controlled host tools. Ten methods can run directly through the unchanged engine; results collect in the active report. A visible context selector and Lessons only option control application sharing. See the [context integration and limits](Learn/README.md#working-with-open-data-and-results).
