...
Teaching rationale: ${item.explanation}` : "";
  ...
  var loaded = false;
  var settings = { configured: false, service: "", label: "Awaiting service activation" };
  var pending = null;
  var revision = 0;
  var feedback = null;
  ...
  function renderStudy() {
    const l = lesson();
    $("main").innerHTML = `<section class="intro"><div class="eyebrow">${esc(l.topic)}</div><h1>${esc(l.title)}</h1><p>${esc(l.summary)}</p></section><div class="actions"><button class="primary" data-action="example">Try in StatsDirect</button><button data-action="r">Explore in R</button><button class="link-button" data-action="help">Read help \u2197</button></div><details><summary>Your worked example \xB7 fictional data</summary><p>${esc(l.steps)}</p></details><section class="chat"><div class="chat-heading"><strong>Your biostatistics tutor</strong><span class="badge">${esc(settings.label)}</span></div><div id="messages" class="messages" role="log" aria-label="Learning conversation"></div><div class="composer"><label for="question">Ask a question, explain your thinking, or paste a small R example</label><textarea id="question" rows="2" maxlength="4000" placeholder="For example: why do we analyse the differences?" ${pending ? "disabled" : ""}>${esc(state.draft)}</textarea><div class="composer-bottom"><small>${settings.configured ? "Send shares learning context and up to 40 recent messages with your tutor service and OpenAI. Use fictional data; avoid identifiers." : "Your course provider manages tutor access. No OpenAI account or key is needed. The shared service is awaiting activation; lessons and practice are available now."}</small><button id="send" class="primary" ${pending ? "disabled" : ""}>${settings.configured ? "Send" : "Tutor connection"}</button>${pending ? '<button id="stop">Stop</button>' : ""}</div><p class="small" id="chatStatus" role="status">${pending ? "The tutor is thinking\u2026" : ""}</p></div></section><div class="suggestions"><button data-prompt="Explain this without assuming I know any statistics.">Explain simply</button><button data-prompt="Ask me one original exam-style question on this topic, then wait for my answer.">Test my understanding</button><button data-prompt="Walk me through the bundled R example line by line, and suggest one small change I can try.">Help me learn R</button></div>`;
  ...
  function configure() {
    ...
    else notice("Use Tutor connection in the Mac application. Your course provider manages access; no OpenAI key is needed.");
  }
  ...
  if (native) post({ action: "ready" });
    ...
    notice("Browser preview. Use the Mac app for the shared tutor connection, analyses, R sessions and email drafts.");
  }
})();
