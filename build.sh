...
# STATSDIRECT_ENGINE_PREBUILT: docker-build.sh published the engine already,
# so no .NET SDK is needed here.
if [[ -z "${STATSDIRECT_ENGINE_PREBUILT:-}" ]]; then
  ...
  if [[ -z "$DOTNET_BIN" ]]; then
    printf 'Install the .NET 10 SDK, set DOTNET to its executable, or run ./docker-build.sh instead.\n' >&2
    exit 1
  fi
